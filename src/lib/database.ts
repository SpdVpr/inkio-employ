...
export const updateWorkLocation = async (
  ...
  try {
    // Pokus se aktualizovat pouze workLocation
    await updateDoc(taskRef, {
      workLocation,
      updatedAt: Timestamp.now()
    });
...
// Fetch weekly statistics for all employees
...
export const subscribeToWeeklyStats = (
  startDate: string,
  endDate: string,
  callback: (stats: Record<string, WeeklyStats>) => void
) => {
  const q = query(
    collection(db, COLLECTION_NAME),
    where('taskDate', '>=', startDate),
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
      ...
      if (!statsMap[task.employeeName]) {
        statsMap[task.employeeName] = { totalTasks: 0, completedTasks: 0, completionRate: 0 };
      }

      const subTasks = migrated.subTasks || [];
      subTasks.forEach(st => {
        statsMap[task.employeeName].totalTasks++;
        if (st.status === 'completed') {
          statsMap[task.employeeName].completedTasks++;
        }
      });
    });
    ...
    Object.keys(statsMap).forEach(name => {
      ...
      s.completionRate = s.totalTasks > 0 ? Math.round((s.completedTasks / s.totalTasks) * 100) : 0;
    });
